...
export default {
  ...
  created() {
    // 密码
    let passwordVal = FileUtil.readConfigFile("password");
    let owner = FileUtil.whoami;
    if (!passwordVal) {
      this.show = true;
    } else {
        this.val = passwordVal;
      FileUtil.changeOwner(passwordVal, owner, (err, stdout, stderr) => {
        if (err) {
          this.show = true;
        } else {
            this.show=false;
          FileUtil.changeOwner(passwordVal, "root", function() {});
          // 写入 host
          FileUtil.writeHost();
        }
      });
    ...
    inputHandle(e) {
      ...
      FileUtil.changeOwner(val, owner, (err, stdout, stderr) => {
        if (err) {
          ...
          this.show = false;
          FileUtil.writeConfigFile("password", val);
          FileUtil.changeOwner(val, "root", function() {});
        }
      });
    }
  }
